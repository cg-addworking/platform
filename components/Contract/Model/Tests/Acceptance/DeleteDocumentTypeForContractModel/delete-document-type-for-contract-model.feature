#language: fr
Fonctionnalité: Delete document for a steakholder
  Contexte:
    ...
    Et que les parties prenantes suivantes existent
      ...
      | 2                     | 3      | party contract model 2 order 3 | 3     |
      | 2                     | 4      | party contract model 2 order 4 | 4     |
      | 3                     | 5      | party contract model 3 order 5 | 5     |
      | 3                     | 6      | party contract model 3 order 6 | 6     |

    Et que les documents types de model de contract suivants existent
      | number | document_type_display_name | contract_model_party_order | party_contract_model_number | validation_required |
      | 1      | DOC 1                      | 1                          | 1                           | 0                   |
      | 2      | DOC 1                      | 3                          | 2                           | 1                   |
      | 3      | DOC 1                      | 6                          | 3                           | 1                   |

  @scenario1
  Scénario: Supprimer un document type pour une partie prenante d'un modele de contrat
    Etant donné que je suis authentifié en tant que utilisateur avec l'email "antoine.pierre@addwun.com"
    Quand j'essaie de supprimer le document type "1" associe a la partie prenante numero "1" du modele de contrat numero "1"
    Alors Le contract model document type numero "1" n'existe plus
    Alors La partie prenante numero "1" du modele de contrat numero "1" possede toujours deux document type

  @scenario2
  Scénario: Impossibilité de supprimer un document type pour une partie prenante d'un modele de contrat si l'utilisateur n'est pas support
    Etant donné que je suis authentifié en tant que utilisateur avec l'email "jean.paul@clideux.com"
    Quand j'essaie de supprimer le document type "2" associe a la partie prenante numero "3" du modele de contrat numero "2"
    Alors une erreur est levée car l'utilisateur connecté n'est pas support

  @scenario3
  Scénario: Impossibilité de supprimer un document type pour une partie prenante d'un modele de contrat s'il est publié
    Etant donné que je suis authentifié en tant que utilisateur avec l'email "antoine.pierre@addwun.com"
    Quand j'essaie de supprimer le document type "2" associe a la partie prenante numero "3" du modele de contrat numero "2"
  ...
  Scénario: Impossibilité de definir un document type pour une partie prenante d'un modele de contrat s'il est archivé
    Etant donné que je suis authentifié en tant que utilisateur avec l'email "antoine.pierre@addwun.com"
    Quand j'essaie de supprimer le document type "3" associe a la partie prenante numero "6" du modele de contrat numero "3"
