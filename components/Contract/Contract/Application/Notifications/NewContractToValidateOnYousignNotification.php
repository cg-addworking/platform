...
class NewContractToValidateOnYousignNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->subject(
                ($this->is_followup
                ? __(
                    'components.contract.contract.application.views.contract.mail'
                        .'.contract_to_validate_on_yousign.followup'
                ) : '')
                . __(
                    'components.contract.contract.application.views.contract.mail'
                    .'.contract_to_validate_on_yousign.subject'
                )
            )
            ->markdown('contract::contract.mail.new_contract_to_validate_on_yousign', [
                'contract_name' => $this->contract->getName(),
                'owner' => $this->contract->getEnterprise()->name,
                'url' => domain_route(
                    route(
                        'contract.validate',
                        [$this->contract, $this->party_to_notify]
                    ),
