<?php
namespace Components\Billing\PaymentOrder\Tests\Acceptance\MarkPaymentOrderAsPaid;
...
class MarkPaymentOrderAsPaidContext extends TestCase implements Context
{
    use RefreshDatabase;
    ...
    /**
     * @Given /^les fichiers des ordres de paiement suivants existent$/
     */
    public function lesFichiersDesOrdresDePaiementSuivantsExistent(TableNode $files)
    {
        foreach ($files as $item) {
            $file = factory(File::class)->create();

            $paymentOrder = $this->paymentOrderRepository->findbyNumber($item['number']);
            $paymentOrder->file()->associate($file->id);
            $paymentOrder->save();
        }
    }
    ...
    /**
     * @When /^j\'essaie de mettre le statut payé à l\'ordre de paiement numero "([^"]*)"$/
     */
    public function jessaieDeMettreLeStatutPayeALordreDePaiementNumero(int $number)
    {
        $auth_user = $this->userRepository->connectedUser();
        $payment_order = $this->paymentOrderRepository->findByNumber($number);

        try {
            $this->response = (new MarkPaymentOrderAsPaid(
                $this->userRepository,
    ...
    /**
     * @Then /^l\'ordre de paiement est au status payé$/
     */
    public function lordreDePaiementEstAuStatusPaye()
    {
        $this->assertEquals($this->response->getStatus(), PaymentOrder::STATUS_EXECUTED);
    }
    ...
    /**
     * @Then /^une erreur est levée car l\'ordre de paiement n\'a pas de fichier$/
     */
    public function uneErreurEstLeveeCarLordreDePaiementNaPasDeFichier()
    {
        $this->assertContainsEquals(
            PaymentOrderFileNotFoundException::class,
            $this->errors
        );    }
