...
use Components\Billing\Outbound\Domain\Exceptions\OutboundInvoiceIsPublishedException;
...
class PublishOutboundInvoice implements ShouldQueue
{
    ...
    public function handle(?User $auth_user, ?OutboundInvoiceInterface $outbound_invoice)
    {
        ...
        return $this->outboundInvoiceRepository->publish($outbound_invoice);
    }
    ...
    public function checkOutboundInvoice($outbound_invoice)
    {
        ...
        if ($this->outboundInvoiceRepository->isPublished($outbound_invoice)) {
            throw new OutboundInvoiceIsPublishedException;
        }
    }
}
