#language: fr
Fonctionnalité: show document type model
  Contexte:
    ...
    Et que les documents types model suivants existent
      | type            | display_name          | description | content      | short_id |
      | document type 1 | document type model 1 | Attestation | blablablabla | 1        |
      | document type 2 | document type model 1 | Attestation | blablablabla | 2        |
      | document type 2 | document type model 2 | Attestation | blablablabla | 3        |
      | document type 2 | document type model 3 | Attestation | blablablabla | 4        |

  @scenario1
  Scénario: Consulter un document type model d'un document type
    Etant donné que je suis authentifié en tant que utilisateur avec l'email "antoine.pierre@addwun.com"
    Quand j'essaie de consulter le document type model numéro "1"
    Alors le document type model numéro "1" est consultable

  @scenario2
  Scénario: Impossibilité de consulter le document type model d'un document type sans être membre du support
