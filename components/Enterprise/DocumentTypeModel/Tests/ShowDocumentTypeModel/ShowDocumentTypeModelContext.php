...
namespace Components\Enterprise\DocumentTypeModel\Tests\ShowDocumentTypeModel;
...
class ShowDocumentTypeModelContext extends TestCase implements Context
{
    use RefreshDatabase;
    ...
    /**
     * @When /^j\'essaie de consulter le document type model numéro "([^"]*)"$/
     */
    public function jessaieDeConsulterLeDocumentTypeModelNumero(string $short_id)
    {
        $auth_user = $this->userRepository->connectedUser();

        $document_type_model = $this->documentTypeModelRepository->findByShortId($short_id);

        try {
            $this->response = (
            new ShowDocumentTypeModel($this->userRepository)
    ...
    /**
     * @Then /^le document type model numéro "([^"]*)" est consultable$/
     */
    public function leDocumentTypeModelNumeroEstConsultable(string $short_id)
    {
        $document_type_model = $this->documentTypeModelRepository->findByShortId($short_id);

        $this->assertEquals($document_type_model->getShortId(), $this->response->getShortId());
    }
