#language: fr
Fonctionnalité: publish document type model
  Contexte:
    ...
    Et que les documents types model suivants existent
      ...
      |  1        | document type 1  | document_type_model_1  | document type model 1 | Attestation | blablablabla | null         |
      |  2        | document type 2  | document_type_model_2  | document type model 1 | Attestation | blablablabla | null         |
  ...
  @scenario1
  Scénario: publier un document type model
    Etant donné que je suis authentifié en tant que utilisateur avec l'email "peter.parker@addworking.com"
    Quand j'essaie de publier le document type model numéro "1"
    Alors le document type model numéro "1" est publié

  @scenario2
  Scénario: Impossibilité de publier un document type model pour un document type si l'utilisateur n'est pas support
    Etant donné que je suis authentifié en tant que utilisateur avec l'email "tony.stark@client.com"
    Quand j'essaie de publier le document type model numéro "2"
    Alors une erreur est levée car l'utilisateur connecté n'est pas support

  @scenario3
    Scénario: Impossibilité de publier un document type model pour un document type s'il est publié
      Etant donné que je suis authentifié en tant que utilisateur avec l'email "peter.parker@addworking.com"
      Quand j'essaie de publier le document type model numéro "3"
      Alors une erreur est levée car le document type model est publié