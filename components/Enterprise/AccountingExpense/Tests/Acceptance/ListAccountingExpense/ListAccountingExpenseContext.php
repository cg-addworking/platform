...
namespace Components\Enterprise\AccountingExpense\Tests\Acceptance\ListAccountingExpense;
...
class ListAccountingExpenseContext extends TestCase implements Context
{
    use RefreshDatabase;
    ...
    private $accountingExpenseListPresenter;
    ...
    public function __construct()
    {
        ...
        $this->accountingExpenseListPresenter = new AccountingExpenseListPresenter;
    ...
    /**
     * @When /^j\'essaie de lister les postes de dépense pour l\'entreprise avec le siret numéro "([^"]*)"$/
     */
    public function jessaieDeListerLesPostesDeDepensePourLentrepriseAvecLeSiretNumero($siret)
    {
        $authenticated = $this->userRepository->connectedUser();
        $enterprise = $this->enterpriseRepository->findBySiret($siret);

        try {
            $this->response = (new ListAccountingExpense(
                $this->accountingExpenseRepository,
                $this->memberRepository,
                $this->userRepository
            ))->handle($this->accountingExpenseListPresenter, $authenticated, $enterprise);
        } catch (Exception $e) {
    ...
    /**
     * @Then /^les postes de dépense sont listés$/
     */
    public function lesPostesDeDepenseSontListes()
    {
