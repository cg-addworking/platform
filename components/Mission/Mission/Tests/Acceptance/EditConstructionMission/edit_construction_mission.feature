#language: fr
Fonctionnalité: modifier une mission 
  Contexte:
    Étant donné les secteurs suivants  existent
      | number | name         | display_name |
      | 1      | construction | Construction |

    Et que les entreprises suivantes existent
      | siret          | client_siret     | name       | is_customer | is_vendor | sector_number |
      | 01000000000000 | null             | Addworking | 0           | 0         | 1             |
      | 03000000000000 | null             | client 2   | 1           | 0         | 1             |
      | 04000000000000 | null             | client 2   | 1           | 0         | 1             |
      | 05000000000000 | 03000000000000   | vendor 1   | 0           | 1         | null          |
      | 06000000000000 | 03000000000000   | vendor 2   | 0           | 1         | null          |
      | 07000000000000 | 03000000000000   | vendor 3   | 0           | 1         | null          |

    Et que les utilisateurs suivants existent
      | email                          | firstname | lastname   | is_system_admin | siret          | number |
      | peter.parker@addworking.com    | Peter     | PARKER     | 1               | 01000000000000 | 1      |
      | clark.kent@client3.com         | Clark     | KENT       | 0               | 03000000000000 | 2      |
      | natasha.roumanouff@client4.com | Natasha   | ROUMANOUFF | 0               | 04000000000000 | 3      |
      | bruce.wayne@client5.com        | Bruce     | WAYNE      | 0               | 03000000000000 | 4      |
      | tony.stark@vendor.com          | Tony      | STARK      | 0               | 05000000000000 | 5      |
      | jarvis.stark@vendor2.com       | Jarvis    | STARK      | 0               | 06000000000000 | 6      |
      | john.smith@vendor3.com         | John      | SMITH      | 0               | 07000000000000 | 7      |

    Et que les chantiers suivants existent
      | number | name        | description  | estimated_budget | started_at | ended_at   | owner_siret    | created_by              |
      | 1      | Chantier n1 | description1 | 123456789.98     | 2021-02-20 | 2021-12-22 | 03000000000000 | clark.kent@client3.com  |
      | 2      | Chantier n2 | description2 | 123456709.98     | 2022-02-20 | 2022-12-22 | 03000000000000 | clark.kent@client3.com  |

    Et que les missions suivantes existent
      | number   | name          | referent_number  | enterprise_siret   | label   | starts_at    |  ends_at     | description   | external_id | analytic_code | status          | workfield_number | created_by_number | vendor_number |
      |    1     | mission1      | 2                | 03000000000000     | lable1  | 2021-04-15   |  2022-04-30  |  mission1     |   id1       |  D1120        | draft           |    1             |         1         |   5           |
      |    2     | mission2      | 4                | 03000000000000     | lable2  | 2021-04-16   |  2022-04-30  |  mission2     |   id2       |  D1121        | draft           |    2             |         2         |   6           |
      |    3     | mission2      | 2                | 03000000000000     | lable2  | 2021-04-10   |  2021-04-13  |  mission3     |   id3       |  D1122        | in_progress     |   null           |         4         |   7           |

  @scenario1
  Scénario: Modifier une mission en tant que support
    Etant donné que je suis authentifié en tant que utilisateur avec l'email "peter.parker@addworking.com"
    Quand j'essaie de modifier la mission numéro "1"
    Alors la mission numéro "1" est modifiée

  @scenario2
  Scénario: Modifier une mission en tant qu'entreprise propriétaire
    Etant donné que je suis authentifié en tant que utilisateur avec l'email "clark.kent@client3.com"
    Quand j'essaie de modifier la mission numéro "2"
    Alors la mission numéro "2" est modifiée

  @scenario3
  Scénario: Impossibilité de modifier une mission en tant qu'utilisateur si l'entreprise n'est pas cliente
    Etant donné que je suis authentifié en tant que utilisateur avec l'email "tony.stark@vendor.com"
    Quand j'essaie de modifier la mission numéro "2"
    Alors une erreur est levée car l'entreprise n'est pas cliente

  @scenario4
  Scénario: Impossibilité de modifier une mission si son état est différent de brouillon ou bon pour démarrage
    Etant donné que je suis authentifié en tant que utilisateur avec l'email "bruce.wayne@client5.com"
    Quand j'essaie de modifier la mission numéro "3"
    Alors une erreur est levée car la mission n'est pas en brouillon ou bon pour démarrage