...
namespace Components\Mission\Offer\Tests\Acceptance\ShowConstructionOffer;
...
class ShowConstructionOfferContext extends TestCase implements Context
{
    use RefreshDatabase;
    ...
    public function __construct()
    {
        ...
        $this->offerShowPresenter = new OfferShowPresenter;
    }
    ...
    /**
     * @When j'essaie de voir le détail de l'offre de mission numéro :arg1
     */
    public function jessaieDeVoirLeDetailDeLoffreDeMissionNumero($number)
    {
        ...
        try {
            $this->response = (new ShowConstructionOffer(
                $this->proposalRepository,
                $this->offerRepository,
                $this->userRepository
            ))->handle($this->offerShowPresenter, $user, $offer);
        } catch (Exception $e) {
    ...
    /**
     * @Then le détail de l'offre de mission numéro :arg1 est affiché
     */
    public function leDetailDeLoffreDeMissionNumeroEstAffiche($number)
    {
        $offer = $this->offerRepository->findByNumber($number);

        $this->assertEquals($offer->getNumber(), $this->response['number']);
    }
    ...
    public function uneErreurEstLeveeCarLentrepriseNestPasProprietaire()
    {
        self::assertContainsEquals(
            UserHasNotPermissionToShowOfferException::class,
            $this->errors
        );
    }

    /**
     * @Then une erreur est levée car l'offre n'a pas de propositions
     */
    public function uneErreurEstLeveeCarLoffreNaPasDePropositions()
    {
        self::assertContainsEquals(
            UserHasNotPermissionToShowOfferException::class,
            $this->errors
    ...
    /**
     * @Then une erreur est levée car l'offre de mission n'existe pas
     */
    public function uneErreurEstLeveeCarLoffreDeMissionNexistePas()
    {
