@component('components.form.modal', [
    'id'     => 'passwork-qualified-modal',
    'action' => route('sogetrel.passwork.status', $passwork),
])
    @slot('title')
        {{ __('sogetrel.user.passwork.modals._qualified_passwork.title') }}
    @endslot

    @csrf
    @method('put')

    <input type="hidden" name="status" value="{{ sogetrel_passwork()::STATUS_PREQUALIFIED }}">
    ...
    @form_group([
        'text'     => __('sogetrel.user.passwork.modals._qualified_passwork.comments'),
        'type'     => "textarea",
